...
public class EmployeeServiceImpl extends ServiceImpl<EmployeeMapper, Employee> implements EmployeeService {
    ...
    @Override
    public Page<Employee> selectForList(QueryObject qo) {
        queryWrapper.clear();
        if (qo.getKeyword() != null) {
            queryWrapper.like(Employee::getUsername, qo.getKeyword()).or().like(Employee::getName,qo.getKeyword());
        }
        Page<Employee> employeePage = baseMapper.selectPage(new Page<>(qo.getCurrent(), qo.getLimit()), queryWrapper);
        return employeePage;
    }
}
